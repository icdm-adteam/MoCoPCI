...
def parse_args():
    ...
    parser.add_argument('--scene_list', type=str,
                        default='',
                        help='Path of the scene list to be used in the dataset.')
...
def test(args):
    ...
    with torch.no_grad():
        ...
        for i, (input, gt) in pbar:
            ...
            for j in range(len(time_inp)):
                ...
                if t != 0.5:
                    pass
                s_time = time.time()
                pred_pc = net( input[1], input[2], gt, time_inp, True)
                e_time = time.time()
        ...
        chamfer_loss_array = np.array(chamfer_loss_list, dtype=object)
        emd_loss_array = np.array(emd_loss_list, dtype=object)
        ...
        # np.save(f'chamfer_loss_data.npy-scene02-{mean_chamfer_loss}', chamfer_loss_array)
        # np.save(f'emd_loss_data.npy-scene02-{mean_emd_loss}', emd_loss_array)
