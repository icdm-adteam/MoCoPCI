...
from models.pointconv_util import CrossLayerLightFeatCosine as CrossLayer, FlowEmbeddingLayer, \
    BidirectionalLayerFeatCosine, SceneFlowEstimatorResidualUncertrainty
from models.pointconv_util import SceneFlowEstimatorResidual
from models.pointconv_util import index_points_gather as index_points, index_points_group, Conv1d, square_distance, knn_point_cosine, knn_point, exists, default, extract, cosine_beta_schedule, SinusoidalPosEmb
import time# from models.amformer import MotionFormer

import  random
import numpy as np
def random_choice(option_a, option_b, probability=0.5):
    return option_a if random.random() < probability else option_b
...
class RecurrentUnit(nn.Module):
    def __init__(self, iters, feat_ch, feat_new_ch, latent_ch, cross_mlp1, cross_mlp2,
        ...
        self.iters = iters
        self.scale = scale
        self.recurrent2 = RecurrentUnit(iters=iters, feat_ch=128, feat_new_ch=128, latent_ch=64  + 64*4, cross_mlp1=[128, 128], cross_mlp2=[128, 128], weightnet=weightnet, flow_channels = [64, 64], flow_mlp = [32, 32])
        ...
        # add
        neighbors = 9
        self.flow = DiffusionSceneFlowGRUResidual(neighbors, in_channel=cross_mlp2[-1] + feat_ch,
                                                  latent_channel=latent_ch, mlp=flow_channels, channels=flow_channels)
        self.warping = PointWarping()
        self.downsample = Conv1d(latent_ch, 64)

    def forward(self, pc1, pc2, feat1_new, feat2_new, feat1, feat2, up_flow, up_feat, t, gt_flow=None, certainty=None,
                uncertainty=0.5, train=True, l2_impute_flow=None):
        ...
        for i in range(self.iters):
            ...
            if train:
                feat_flow, flow, certainty_new, loss = self.flow(pc1, pc1, up_feat, new_feat1, up_flow, gt_flow, t,
                                                                 certainty, uncertainty, train)
            else:
                feat_flow, flow, certainty_new = self.flow(pc1, pc1, up_feat, new_feat1, up_flow, gt_flow, t,certainty,
                                                           uncertainty, train)
            up_flow = flow
            up_feat = feat_flow
            c_feat1 = torch.cat([feat1, feat1_new], dim=1)
        ...
        if train:
            return flows, feat1_new, feat2_new, feat_flow, certainty_new, loss, fe
        else:
            return flows, feat1_new, feat2_new, feat_flow, certainty_new, fe
...
class DiffusionSceneFlowGRUResidual(nn.Module):
    def __init__(self, nsample, in_channel, latent_channel, mlp, mlp2=None, bn = False, use_leaky = True,\
        ...
        mlp = [latent_channel, latent_channel]
        self.fc = nn.Conv1d(mlp[-1]+3,4,1)
        self.clamp = clamp
        #last_channel = in_channel + 3
        last_channel = in_channel + 3 + 64 + 3 + 1 + 64
        ...
        self.fuse_x = nn.Conv1d(latent_channel, mlp[0], 1, bias=False)
        ...
        if mlp2:
            ...
            for out_channel in mlp2:
                self.mlp2.append(Conv1d(last_channel+64, out_channel, 1, bias=False, bn=bn))
                last_channel = out_channel
    ...
    def denoise(self, xyz1, xyz2, points1, points2, delta_flow, flow_new, delta_certainty, certainty_new, t):
        B, C, N1 = xyz1.shape
        _, _, N2 = xyz2.shape
        ...
        for i in range(self.iters):
            ...
            if self.radius is None:
                ...
                time_pred = torch.tensor(T).repeat(batch_size, n, 1).cuda().float().detach()
                time_pred = self.posenc(time_pred)
                time_pred_up = self.time_mlp_2(time_pred)
                time_pred_up = time_pred_up.unsqueeze(-2).repeat(1, 1, self.nsample, 1)
                ...
                new_points = torch.cat(
                    [grouped_points2, direction_xyz, delta_certainty, delta_flow, time, time_pred_up],
                    dim=-1)  # B, N1, nsample, D1+D2+3
                #
                # 特征聚合， 动量聚合， 不确定性， 微分流， 时间, 帧时间
            ...
            new_points_delta = torch.cat((new_points_delta, time_pred.permute(0, 2, 1)), dim=1)
    ...
    def add_noise(self, flow, certainty, flow_gt, uncertainty=0.5):
        ...
        gt_certainty = torch.where(torch.logical_or(gt_certainty_norm < uncertainty,
        ...
        # 将 gt_delta_flow 中的无穷大值替换为 0
        gt_delta_flow = gt_delta_flow.detach()
        # 随机选择一个时间步 t，用于给定批次的每个样本
        t = torch.randint(0, self.timesteps, (batch_size,), device=flow.device).long()
        # 为 gt_delta_flow 和 gt_delta_certainty 添加随机噪声，以增强模型鲁棒性
        ...
        # 使用 self.q_sample 函数生成带噪声的 delta_flow 和 delta_certainty
        delta_flow = self.q_sample(x_start=gt_delta_flow, t=t, noise=noise)
        # 加噪
        flow_new = flow + delta_flow  # 更新 flow
        # 为什么是flow不是gt_flow？
        delta_certainty = self.q_sample(x_start=gt_delta_certainty, t=t, noise=noise_certainty)
        certainty_new = certainty + delta_certainty  # 更新 certainty
        return
    def forward(self, xyz1, xyz2, points1, points2, flow, flow_gt, T, certainty, uncertainty=0.5, train = True):
        ...
        if train:
            # add_noise
            delta_flow, gt_delta_flow, flow_new, delta_certainty, gt_delta_certainty, certainty_new, t = self.add_noise(flow, certainty, flow_gt)
            # 降噪
            new_points, flow_new, certainty_new, loss = self.denoise(delta_flow, flow_new, delta_certainty, certainty_new, t)
            ...
            for time, time_next in time_pairs:
                ...
                for i in range(self.iters):
                    ...
                    if self.radius is None:
                        ...
                        new_points = torch.cat([grouped_points2, direction_xyz, delta_certainty, delta_flow, time, time_pred_up], dim = -1) # B, N1, nsample, D1+D2+3
                        new_points = new_points.permute(0, 3, 2, 1) # [B, D2+3, nsample, N1]
...
class FlowRefineNet_Unet(nn.Module):
    ...
    def forward(self, encoder, pc1s, pc2s, feat1s, feat2s, flows, k, t):
        warped_pc1t_l3 = pc1s[3] + flows[3] * t
        warped_pc1t_l2 = pc1s[2] + flows[2] * t
        warped_pc1t_l1 = pc1s[1] + flows[1] * t
        warped_pc2t_l3 = pc2s[3] - flows[3] * (1 - t)
        warped_pc2t_l2 = pc2s[2] - flows[2] * (1 - t)
        warped_pc2t_l1 = pc2s[1] - flows[1] * (1 - t)
        warped_pc1t = pc1s[0] + flows[0] * t
        warped_pc2t = pc2s[0] - flows[0] * (1 - t)
        warped_pc1_lst = [warped_pc1t, warped_pc1t_l1, warped_pc1t_l2, warped_pc1t_l3]
        warped_pc2_lst = [warped_pc2t, warped_pc2t_l1, warped_pc2t_l2, warped_pc2t_l3]
        ...
        warped_feat2t_l0 = feat2s[0] - (
            F.interpolate(flows[0].permute(0, 2, 1), size=feat2s[0].size(1), mode="area")).permute(0, 2, 1) * (1-t)
        ...
        warped_feat2t_l0 = self.rlevel0(warped_feat2t_l0)
        # fused_down1, fused_feat1, _ = encoder.level1(warped_pc1t, warped_feat1t_l0)
        fused_down1, fused_feat1 = self.level1(warped_pc1t, warped_feat1t_l0)
        fused_down2, fused_feat2 = self.level1(warped_pc2t, warped_feat2t_l0)
        fea_shape1 = self.shape1(fused_feat1.permute(0, 2, 1), fused_down1.permute(0, 2, 1))  # [B,64,2048]
        fea_shape2 = self.shape1(fused_feat2.permute(0, 2, 1), fused_down2.permute(0, 2, 1))  # [B,64,2048]
        ...
        up_feat0_1 = self.upsample(warped_pc1t, fused_down1, fea_shape1)
        up_feat0_2 = self.upsample(warped_pc2t, fused_down2, fea_shape2)
        refine_out_1 = (self.pred(up_feat0_1.permute(0, 2, 1))).permute(0, 2, 1)
        refine_out_2 = (self.pred(up_feat0_2.permute(0, 2, 1))).permute(0, 2, 1)


        # return warped_pc1t, warped_pc2t, warped_pc1t_l1, warped_pc1t_l2, warped_pc1t_l3, refine_out
        return warped_pc1_lst, warped_pc2_lst, refine_out_1, refine_out_2
...
class Estimatier(nn.Module):
    def __init__(self, scale=1.0, iters=3):
        ...
        self.recurrent1 = RecurrentUnit(iters=iters, feat_ch=64, feat_new_ch=64, latent_ch=64 + 32*4, cross_mlp1=[64, 64], cross_mlp2=[64, 64], weightnet=weightnet, flow_channels = [64, 64], flow_mlp = [32, 32])
        ...
        self.flow3 = SceneFlowEstimatorResidualUncertrainty(256 + 128*4, 256, channels=[128, 64], mlp=[], weightnet=weightnet)
    ...
    def forward(self, pc1s, pc2s, feat1s, feat2s, af, mf, t, gtgt=None, train=False, uncertainty=0.5):
        # xyz1, xyz2: B, N, 3
        # color1, color2: B, N, 3
        B = pc1s[0].shape[0]
        ...
        # for index, (maf, mmf) in enumerate(zip(af, mf)):
        #     amf.append(torch.cat( [feat1s[index] , t * mf[index][:B], (1 - t) * mf[index][B:], af[index][:B], af[index][B:]] , dim=1))
            # amf.append(torch.cat([maf, mmf], dim=1))
        # l4
        ...
        feat3, flows3, certainty3 = self.flow3(pc1s[3], torch.cat( [feat1s[3] , t * mf[3][:B],(1-t) * mf[3][B:], af[3][:B], af[3][B:]] , dim=1), cross3)
        ...
        # flows2, feat1_new_l2, feat2_new_l2, feat2 = self.recurrent2(pc1s[2], pc2s[2], feat1_l3_2, feat2_l3_2, feat1s[2],
        #                                                             feat2s[2], up_flow2, up_feat2)
        ...
        if train:
            l2_flow_gt = gtgt[2]
            flows2, feat1_new_l2, feat2_new_l2, feat2 , certainty2, loss_l2, cost2 = self.recurrent2(pc1s[2], pc2s[2], feat1_l3_2, feat2_l3_2, feat1s[2], feat2s[2], up_flow2, torch.cat( [up_feat2,  mf[2][:B], mf[2][B:], af[2][:B], af[2][B:]] , dim=1), t, l2_flow_gt, up_certainty2, uncertainty, train)
        else:
            l2_flow_gt = None
            flows2, feat1_new_l2, feat2_new_l2, feat2 ,certainty2, cost2 = self.recurrent2(pc1s[2], pc2s[2], feat1_l3_2, feat2_l3_2, feat1s[2], feat2s[2], up_flow2, torch.cat( [up_feat2, mf[2][:B], mf[2][B:], af[2][:B], af[2][B:]] , dim=1), t, l2_flow_gt, up_certainty2, uncertainty, train)
        flows2 = flows2[::-1][0]
        ...
        if train:
            l1_flow_gt = gtgt[1]
            flows1, feat1_new_l1, feat2_new_l1, feat1 , certainty1, loss_l1, cost1 = self.recurrent1(pc1s[1], pc2s[1], feat1_l2_1, feat2_l2_1, feat1s[1], feat2s[1], up_flow1, torch.cat( [up_feat1, mf[1][:B], mf[1][B:], af[1][:B], af[1][B:]] , dim=1), t, l1_flow_gt, up_certainty1, uncertainty, train)
        else:
            l1_flow_gt = None
            flows1, feat1_new_l1, feat2_new_l1, feat1 ,certainty1, cost1 =  self.recurrent1(pc1s[1], pc2s[1], feat1_l2_1, feat2_l2_1, feat1s[1], feat2s[1], up_flow1, torch.cat( [up_feat1, mf[1][:B], mf[1][B:], af[1][:B], af[1][B:]] , dim=1), t, l1_flow_gt, up_certainty1, uncertainty, train)
        flows1 = flows1[::-1][0]
        ...
        if train:
            l0_flow_gt = gtgt[0]
            flows0, feat1_new_l0, feat2_new_l0, feat0 , certainty0, loss_l0, cost0 = self.recurrent0(pc1s[0], pc2s[0], feat1_l1_0, feat2_l1_0, feat1s[0], feat2s[0], up_flow0, torch.cat( [up_feat0, af[0][:B], af[0][B:]] , dim=1), t, l0_flow_gt, up_certainty0, uncertainty, train)
        else:
            l0_flow_gt = None
            flows0, feat1_new_l0, feat2_new_l0, feat0 ,certainty0, cost0 = self.recurrent0(pc1s[0], pc2s[0], feat1_l1_0, feat2_l1_0, feat1s[0], feat2s[0], up_flow0, torch.cat( [up_feat0, af[0][:B], af[0][B:]] , dim=1), t, l0_flow_gt, up_certainty0, uncertainty, train)
        ...
        # flows = [flows0[::-1][0], flows1[::-1][0], flows2[::-1][0], flows3]
        flows = [flows0, flows1, flows2, flows3]

        # pc1 = pc1s
        # pc2 = pc2s
        if train:
            return flows, loss_l2, loss_l1, loss_l0
        return flows
...
class SceneFlowPWC(nn.Module):
    ...
    def forward(self, xyz1, xyz2, t, gt=None, train=False):
        ...
        if train:
            l0_label = gt - pc1s[0]
            l1_label = downsampling(gt, int(N / 4)) - pc1s[1]
            l2_label = downsampling(gt, int(N / 16)) - pc1s[2]
            l3_label = downsampling(gt, int(N / 32)) - pc1s[3]
            
            gtgt_lst = [l0_label, l1_label, l2_label, l3_label]
        else:
            gtgt_lst = None

        if train:
            flows, loss_l2, loss_l1, loss_l0 = self.inference(pc1s, pc2s, feat1s, feat2s, af, mf, t, gtgt_lst, train)
        else:
            flows = self.inference(pc1s, pc2s, feat1s, feat2s, af, mf, t, gtgt_lst, train)
        # refine
        _, _, N = flows[0].shape
        k = 32

        # list
        warped_pc1t_l3 = pc1s[3] + flows[3]
        warped_pc1t_l2 = pc1s[2] + flows[2]
        warped_pc1t_l1 = pc1s[1] + flows[1]

        warped_pc2t_l3 = pc2s[3] - flows[3]
        warped_pc2t_l2 = pc2s[2] - flows[2]
        warped_pc2t_l1 = pc2s[1] - flows[1]


        warped_pc1t = pc1s[0] + flows[0]
        warped_pc2t = pc2s[0] - flows[0]
        warped_pc1_lst = [warped_pc1t, warped_pc1t_l1, warped_pc1t_l2, warped_pc1t_l3]
        warped_pc2_lst = [warped_pc2t, warped_pc2t_l1, warped_pc2t_l2, warped_pc2t_l3]
        # warped_pc1_lst, warped_pc2_lst, refine_out_1, refine_out_2 = self.refine(self.encoder, pc1s, pc2s, feat1s, feat2s, flows, k=32, t=t)
        ...
        # if t == 0.5:
            # warped_pc = random_choice(warped_pc1_lst[0], warped_pc2_lst[0])
            # refine_out = random_choice(refine_out_1, refine_out_2)
        # if t > 0.5:
            # fused_points = warped_pc2_lst[0]
            # warped_pc = warped_pc2_lst[0]
            # refine_out = refine_out_2
        # else:
        fused_points = warped_pc1_lst[0]
        # fusion
        ...
        if train:
            ...
            gt_list = [gt1, gt2, gt3]
            return flows[0], fused_points, warped_pc1_lst, warped_pc2_lst, gt_list, loss_l2, loss_l1, loss_l0
        else:
            return fused_points
