...
class CrossLayerLightFeatCosineU(nn.Module):
    def __init__(self, nsample, in_channel, mlp1, mlp2, bn=use_bn, use_leaky=True):
        super(CrossLayerLightFeatCosineU, self).__init__()
    ...
    def cross(self, xyz1, xyz2, up_frame, points1, points2, up_feats,knn1, knn2, pos, mlp, bn, nsample=None):
        B, C, N1 = xyz1.shape
        _, _, N2 = xyz2.shape
        _, D1, _ = points1.shape
        _, D2, _ = points2.shape
        xyz1 = xyz1.permute(0, 2, 1)
        xyz2 = xyz2.permute(0, 2, 1)
        up_frame = up_frame.permute(0, 2, 1)
        points1 = points1.permute(0, 2, 1)
        points2 = points2.permute(0, 2, 1)
        up_feats = up_feats.permute(0, 2, 1)
        knn1 = knn1.permute(0, 2, 1)
        knn2 = knn2.permute(0, 2, 1)
        ...
        direction_xyz = neighbor_xyz - xyz1.view(B, N1, 1, C)
        grouped_points2 = torch.cat((index_points_group(points2, knn_idx).permute(0, 3, 2, 1),
                                     index_points_group(points2, knn_idx_p).permute(0, 3, 2, 1)),
                                    -2)  # B, N1, nsample, D2
        grouped_points1 = points1.view(B, N1, 1, D1).repeat(1, 1, nsample, 1).permute(0, 3, 2, 1)
    ...
    def forward(self, pc1, pc2, feat1, feat2, knn1, knn2):
        feat1_new = self.cross(pc1, pc2, self.cross_t11(feat1), self.cross_t22(feat2), knn1, knn2, self.pos1, self.mlp1,
                               self.bn1)
        feat1_new = self.cross_t1(feat1_new)
        feat2_new = self.cross(pc2, pc1, self.cross_t11(feat2), self.cross_t22(feat1), knn2, knn1, self.pos1, self.mlp1,
                               self.bn1)
        feat2_new = self.cross_t2(feat2_new)
...
class BidirectionalLayerFeatCosine(nn.Module):
    ...
    def cross(self, xyz1, xyz2, points1, points2, knn1, knn2, nsample=None):
        ...
        knn_idx = knn_point_cosine(nsample//2,  knn2, knn1) # B, N1, nsample
        _, knn_idx_p, _ = knn_points(xyz2, xyz1, K=nsample//2)
        neighbor_xyz = torch.cat((index_points_group(xyz2, knn_idx), index_points_group(xyz2, knn_idx_p)),-2)
        direction_xyz = neighbor_xyz - xyz1.view(B, N1, 1, C)
        grouped_points2 =  torch.cat((index_points_group(points2, knn_idx).permute(0, 3, 2, 1),
                                      index_points_group(points2, knn_idx_p).permute(0, 3, 2, 1)),-2)# B, N1, nsample, D2
...
class SceneFlowEstimatorResidualUncertrainty(nn.Module):
    def __init__(self, feat_ch, cost_ch, flow_ch=3, channels=[128, 128], mlp=[128, 64], neighbors=9, clamp=[-200, 200],
                 use_leaky=True, weightnet=16):
        super(SceneFlowEstimatorResidualUncertrainty, self).__init__()
        self.clamp = clamp  # 将场景流估计结果限制在指定范围内
        self.use_leaky = use_leaky  # 使用 LeakyReLU 作为激活函数

        # 创建 PointConv 层列表，用于逐层提取点云特征
        self.pointconv_list = nn.ModuleList()
        last_channel = feat_ch + cost_ch  # 初始通道数为特征和代价体积的通道数之和

        for _, ch_out in enumerate(channels):
            # 创建每个 PointConv 层，用于从邻域中提取特征
            pointconv = PointConv(neighbors, last_channel + 3, ch_out, bn=True, use_leaky=True, weightnet=weightnet)
            self.pointconv_list.append(pointconv)
            last_channel = ch_out  # 更新通道数

        # 创建 MLP 层，用于进一步融合和细化特征
        self.mlp_convs = nn.ModuleList()
        for _, ch_out in enumerate(mlp):
            self.mlp_convs.append(Conv1d(last_channel, ch_out))
            last_channel = ch_out

        # 最终全连接层，将特征映射到 4 维输出（3 维流动和 1 维置信度）
    ...
    def forward(self, xyz, feats, cost_volume, flow=None):
        ...
        Args:
            xyz: 点的三维坐标, [B, 3, N]
            feats: 点的特征, [B, C1, N]
            cost_volume: 代价体积, [B, C2, N]
            flow: 上层传递的场景流估计, [B, 3, N]（可选）
        Returns:
            new_points: 更新后的特征, [B, D', N]
            flow: 更新后的场景流, [B, 3, N]
            certainty: 流动估计的置信度, [B, 1, N]
        '''
        # 将特征和代价体积组合，作为初始特征输入
        new_points = torch.cat([feats, cost_volume], dim=1)

        # 通过 PointConv 层提取和细化特征
        for _, pointconv in enumerate(self.pointconv_list):
            new_points = pointconv(xyz, new_points)
        ...
        flow_local = update[:, :3, :].clamp(self.clamp[0], self.clamp[1])  # 提取前三个通道作为场景流，并进行范围限制
        certainty = update[:, 3:, :]  # 提取第四个通道作为置信度

        # 如果输入的场景流为 None，则初始化；否则使用残差更新
        if flow is None:
            flow = flow_local
        else:
            flow = flow_local + flow  # 残差更新场景流
            # flow = flow_local  # 残差更新场景流

        return new_points, flow, certainty

class SceneFlowEstimatorResidualUncertraintyTimes(nn.Module):
    def __init__(self, feat_ch, cost_ch, flow_ch=3, channels=[128, 128], mlp=[128, 64], neighbors=9, clamp=[-200, 200],
                 use_leaky=True, weightnet=16):
        super(SceneFlowEstimatorResidualUncertraintyTimes, self).__init__()
        ...
        dim = 16
        time_dim = 64
        last_channel = feat_ch + cost_ch + time_dim# 初始通道数为特征和代价体积的通道数之和
    ...
    def forward(self, xyz, feats, cost_volume, t, flow=None):
        '''
        Args:
            xyz: 点的三维坐标, [B, 3, N]
            feats: 点的特征, [B, C1, N]
        ...
        '''
        # 将特征和代价体积组合，作为初始特征输入
        t = torch.tensor([t]).to(feats.device).float()
        t = self.time_mlp(t)
        t = t.unsqueeze(1).repeat(2, feats.shape[-1], 1).permute(0, 2, 1)
        new_points = torch.cat([feats, cost_volume, t], dim=1)
        # 通过 PointConv 层提取和细化特征
        for _, pointconv in enumerate(self.pointconv_list):
            new_points = pointconv(xyz, new_points)
...
class SceneFlowEstimatorResidualMultiFrame(nn.Module):
    def __init__(self, feat_ch, cost_ch, flow_ch=3, channels=[128, 128], mlp=[128, 64], neighbors=9, clamp=[-200, 200],
                 use_leaky=True, weightnet=16):
        super(SceneFlowEstimatorResidualMultiFrame, self).__init__()
        self.clamp = clamp
        self.use_leaky = use_leaky
        self.pointconv_list = nn.ModuleList()
        last_channel = feat_ch + cost_ch
        ...
        # Output 3 sets of flows for 3 frames
        self.fc = nn.Conv1d(last_channel, 12, 1)  # 3 frames x (3 xyz + 1 confidence) = 12 channels
    ...
    def forward(self, xyz, feats, cost_volume, flow=None):
        ...
        feats: B C1 N  
        cost_volume: B C2 N
        flow: B 9 N (3 flows for 3 frames)
        '''
        # Combine features and cost volume as initial features
        new_points = torch.cat([feats, cost_volume], dim=1)

        # Process through PointConv layers
        for _, pointconv in enumerate(self.pointconv_list):
            new_points = pointconv(xyz, new_points)
        ...
        # Generate flows and confidences for 3 frames
        update = self.fc(new_points)
        
        # Split into 3 flows and confidences
        flow_local_1 = update[:, :3, :].clamp(self.clamp[0], self.clamp[1])
        flow_local_2 = update[:, 3:6, :].clamp(self.clamp[0], self.clamp[1]) 
        flow_local_3 = update[:, 6:9, :].clamp(self.clamp[0], self.clamp[1])
        certainty_1 = update[:, 9:10, :]
        certainty_2 = update[:, 10:11, :]
        certainty_3 = update[:, 11:12, :]

        # Combine flows
        flow = torch.cat([flow_local_1, flow_local_2, flow_local_3], dim=1)
        certainty = torch.cat([certainty_1, certainty_2, certainty_3], dim=1)

        return new_points, flow, certainty


class SceneFlowEstimatorResidualSimple(nn.Module):

    def __init__(self, feat_ch, cost_ch, flow_ch=3, channels=[128, 128], mlp=[128, 64], neighbors=9, clamp=[-200, 200],
                 use_leaky=True, weightnet=16):
        super(SceneFlowEstimatorResidualSimple, self).__init__()
        self.clamp = clamp
        self.use_leaky = use_leaky
        self.pointconv_list = nn.ModuleList()
        last_channel = feat_ch + cost_ch
